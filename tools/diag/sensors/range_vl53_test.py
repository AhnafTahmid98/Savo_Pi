...
Physical mapping (Robot Savo front bumper):
  - i2c-1 -> Front-Right  (FR)
  - i2c-0 -> Front-Left   (FL)
...
# Logical names for nicer printing / CSV
# NOTE: Locked mapping for Robot Savo:
#   bus 1 = Front-Right (FR)
#   bus 0 = Front-Left  (FL)
BUS_LABELS = {
    1: "FR",  # i2c-1 = Front-Right
    0: "FL",  # i2c-0 = Front-Left
}

def bus_label(bus: int) -> str:
    """Short role label for a bus (FR/FL or fallback)."""
    return BUS_LABELS.get(bus, f"bus{bus}")

def bus_long(bus: int) -> str:
    """Long label with role + i2c bus number, e.g. 'FR (i2c-1)'."""
    return f"{bus_label(bus)} (i2c-{bus})"

def col_label(bus: int) -> str:
    """Column label with role + bus, e.g. 'FR[bus1]'."""
    return f"{bus_label(bus)}[bus{bus}]"
...
def main():
    ...
    label_str = ", ".join([bus_long(bus) for bus in buses])
    print(
    ...
    for bus in buses:
        ...
        try:
            ...
            sensors[bus] = s
            print(f"  - OK: {bus_long(bus)} online")
        except Exception as e:
            print(f"  - FAIL: {bus_long(bus)} init error: {e}", file=sys.stderr)
    ...
    try:
        if args.csv:
            ...
            header = ["t_sec"] \
                     + [f"{col_label(bus)}_cm" for bus in active_buses] \
                     + [f"{col_label(bus)}_f_cm" for bus in active_buses]
            csv_writer.writerow(header)
            print(f"[CSV] Logging to {args.out}")

        # Header line
        cols = " | ".join(
            [f"{col_label(bus)}(cm)" for bus in active_buses]
            + [f"{col_label(bus)}_f(cm)" for bus in active_buses]
        )
        print("\n t(s)  | " + cols + " | alert")
        print("-" * (12 + len(cols) + 8))
