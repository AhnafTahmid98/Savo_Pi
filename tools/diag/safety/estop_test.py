#!/usr/bin/env python3
"""Observe the Robot SAVO external STOP latch without mutating it."""
...
try:
    ...
    if probe.publisher_count == 0:
        status, reason = 'BLOCKED', 'external_stop_publisher_missing'
    elif not probe.samples:
        status, reason = 'FAIL', 'external_stop_not_observed'
    else:
        details['active'] = bool(probe.samples[-1].get('data', False))
        status, reason = 'PASS', 'external_stop_state_observed'
except (ImportError, RuntimeError, ValueError) as exc:
    status, reason, details = 'BLOCKED', 'ros_probe_unavailable', {'error': str(exc)}
raise SystemExit(emit_result('estop', status, reason, details, output=args.output,
