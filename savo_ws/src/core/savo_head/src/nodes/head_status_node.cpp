...
public:
    ...
    semantic_watch_{"semantic"}
  {
    ...
    pan_tilt_sub_ =
      ...
      get_parameter(
        "pan_tilt_state_topic").as_string(),
      rclcpp::QoS(10).reliable(),
      std::bind(
        &HeadStatusNode::on_pan_tilt_state,
        this,
        std::placeholders::_1));
    ...
    scan_sub_ =
      ...
      get_parameter(
        "scan_state_topic").as_string(),
      rclcpp::QoS(10).reliable(),
      std::bind(
        &HeadStatusNode::on_scan_state,
        this,
        std::placeholders::_1));
    ...
    camera_sub_ =
      create_subscription<std_msgs::msg::String>(
      get_parameter(
        "camera_status_topic").as_string(),
      rclcpp::QoS(10).best_effort(),
      std::bind(
        &HeadStatusNode::on_camera_status,
        this,
        std::placeholders::_1));
    ...
    semantic_sub_ =
      create_subscription<std_msgs::msg::String>(
      get_parameter(
        "semantic_confirmations_topic").as_string(),
      rclcpp::QoS(10).reliable(),
      std::bind(
        &HeadStatusNode::on_semantic_confirmation,
        this,
        std::placeholders::_1));
    ...
    dashboard_pub_ =
      ...
      get_parameter(
        "dashboard_text_topic").as_string(),
      rclcpp::QoS(10).reliable());

    health_service_ =
      create_service<std_srvs::srv::Trigger>(
      get_parameter(
        "health_check_service").as_string(),
      std::bind(
    ...
    const auto status_hz =
      std::max(
      0.2,
      get_parameter(
        "status_publish_hz").as_double());

    status_timer_ =
      create_wall_timer(
      seconds_to_period(1.0 / status_hz),
      std::bind(
        &HeadStatusNode::publish_status,
        this));

    RCLCPP_INFO(
      get_logger(),
      "head status node started");
  }
...
private:
  ...
  void on_pan_tilt_state(
    const sensor_msgs::msg::JointState::SharedPtr msg)
  {
    last_joint_state_ = *msg;

    pan_tilt_watch_.update(
      now_s(),
      joint_text(*msg));
  ...
  void on_camera_status(
    const std_msgs::msg::String::SharedPtr msg)
  {
    camera_watch_.update(
      now_s(),
      msg->data);
  ...
  void on_semantic_confirmation(
    const std_msgs::msg::String::SharedPtr msg)
  {
    semantic_watch_.update(
      now_s(),
      msg->data);
  ...
  void publish_status()
  {
    try {
      ...
      const auto camera_report =
        current_camera_report(now_value);

      status.values = {
        kv(
          "pan_tilt_state",
          to_string(
            pan_tilt_watch_.state(
              now_value,
              get_parameter(
                "state_stale_timeout_s").as_double(),
              get_parameter(
                "require_pan_tilt_for_ok").as_bool()))),
        ...
        kv(
          ...
          to_string(
            scan_watch_.state(
              now_value,
              get_parameter(
                "scan_stale_timeout_s").as_double(),
              get_parameter(
                "require_scan_for_ok").as_bool()))),

        kv(
          "camera_state",
          camera_state_text(now_value)),

        kv(
          "camera_reason",
          camera_reason_text(now_value)),
        ...
        kv(
          ...
          camera_report.bool_value(
            "stream_healthy").value_or(false)),
        ...
        kv(
          "camera_calibrated",
          camera_report.bool_value(
            "calibrated").value_or(false)),

        kv(
          "camera_required",
          get_parameter(
            "require_camera_for_ok").as_bool()),

        kv(
          "semantic_state",
          to_string(
            semantic_watch_.state(
              now_value,
              get_parameter(
                "apriltag_detection_stale_timeout_s")
              .as_double(),
              get_parameter(
                "require_apriltag_for_ok").as_bool()))),

        kv(
          "pan_tilt_age_s",
          pan_tilt_watch_.age_s(now_value)),

        kv(
          "scan_age_s",
          scan_watch_.age_s(now_value)),

        kv(
          "camera_age_s",
          camera_watch_.age_s(now_value)),

        kv(
          "semantic_age_s",
          semantic_watch_.age_s(now_value)),

        kv(
          "pan_tilt_text",
          pan_tilt_watch_.last_text),

        kv(
          "scan_text",
          scan_watch_.last_text),

        kv(
          "camera_text",
          camera_watch_.last_text),

        kv(
          "semantic_text",
          semantic_watch_.last_text),

        kv(
          "last_error",
          last_error_.value_or(""))
      };
  ...
  HeadHealthSummary current_health() const
  {
    ...
    components.push_back(
      camera_status_health(
        camera_status_context(now_value)));
    ...
    /*
     * Optional or non-blocking component warnings remain
     * operational and are reported as OK_WITH_WARNINGS.
     *
     * Required missing components and reported component
     * errors are converted to DiagnosticLevel::kError.
     */
  ...
  CameraStatusContext camera_status_context(
    double now_value) const
  {
    CameraStatusContext context;

    context.seen = camera_watch_.seen;
  ...
  CameraStatusReport current_camera_report(
    double now_value) const
  {
    const auto context =
      camera_status_context(now_value);
    ...
    return parse_camera_status_text(
      context.raw_text);
  }

  std::string camera_state_text(
    double now_value) const
  {
    const auto context =
      camera_status_context(now_value);
  ...
  std::string dashboard_text_line(
    const HeadHealthSummary & health) const
  {
...
int main(int argc, char ** argv)
{
  ...
  try {
    auto node =
      std::make_shared<
      savo_head::HeadStatusNode>();

    rclcpp::spin(node);
  } catch (const std::exception & exc) {
    std::cerr
      << "head_status_node failed: "
      << exc.what()
      << std::endl;
