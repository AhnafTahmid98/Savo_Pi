...
class UiRuntimeNode final : public rclcpp::Node
{
public:
  explicit UiRuntimeNode(const rclcpp::NodeOptions & options = rclcpp::NodeOptions());
  ~UiRuntimeNode() override;

  UiRuntimeNode(const UiRuntimeNode &) = delete;
  UiRuntimeNode & operator=(const UiRuntimeNode &) = delete;
...
private:
  ...
  // Runtime-only V2 interaction overlays.
  // Intro/Home/Status/Power/Map/Safety remain the Classic UI.
  v2::VoiceFaceAnimation v2_voice_animation_{};
  std::unique_ptr<v2::VoiceFaceRenderer> v2_voice_renderer_;
  std::unique_ptr<v2::NavigationRenderer> v2_navigation_renderer_;
