...
def draw_status_panel(base):
    ...
    def small_status_icon(icon, cx, cy):
        if icon == 'gear':
            ...
            d.rounded_rectangle(
                (cx - 4, cy - 11, cx + 4, cy + 4), radius=4, outline=CYAN_SOFT, width=3
            )
            ...
            d.rounded_rectangle(
                (cx - 12, cy - 7, cx + 9, cy + 7), radius=2, outline=CYAN_SOFT, width=2
            )
...
def compose_home(robot_frame_name: str) -> Image.Image:
    ...
    if robot_layer is not None:
        ...
        d.text(
            (ROBOT_X + 145, ROBOT_Y + 178), 'ROBOT LAYER MISSING', font=FONT_MED_BOLD, fill=CYAN
        )
...
def generate_runtime_font_atlas(
    ...
    print(f'generated {output_path} size={atlas.width}x{atlas.height}')
