...
UiRuntimeNode::UiRuntimeNode(const rclcpp::NodeOptions & options)
: rclcpp::Node("savo_ui_node", options)
{
  ...
  v2_voice_renderer_ = std::make_unique<v2::VoiceFaceRenderer>(
    config_.screen_width,
    config_.screen_height);

  v2_navigation_renderer_ = std::make_unique<v2::NavigationRenderer>(
    config_.screen_width,
    config_.screen_height);
...
UiRuntimeNode::~UiRuntimeNode()
{
  close_touch();
}

void UiRuntimeNode::declare_parameters()
{
...
void UiRuntimeNode::load_parameters()
{
...
void UiRuntimeNode::load_assets()
{
...
bool UiRuntimeNode::load_robot360_frames()
{
...
bool UiRuntimeNode::load_page_shells()
{
  auto load_shell =
    [this](
...
bool UiRuntimeNode::load_runtime_font_atlases()
{
  auto load_font =
    [this](
    const std::string & file_name,
...
void UiRuntimeNode::configure_home_idle_sequence()
{
...
void UiRuntimeNode::export_preview_frames()  // NOLINT(readability/fn_size)
{
...
void UiRuntimeNode::configure_display()
{
...
void UiRuntimeNode::configure_touch()
{
...
void UiRuntimeNode::close_touch()
{
  if (touch_fd_ >= 0) {
    ::close(touch_fd_);
    touch_fd_ = -1;
...
void UiRuntimeNode::poll_touch_input()
{
...
void UiRuntimeNode::handle_touch_tap(const int x, const int y)
{
...
bool UiRuntimeNode::screen_from_touch(const int x, const int y, UiScreen * screen) const
{
  if (screen == nullptr) {
    return false;
  }
...
bool UiRuntimeNode::status_view_from_touch(
  const int x,
  const int y,
...
void UiRuntimeNode::configure_power_subscriptions()
{
...
void UiRuntimeNode::configure_live_subscriptions()
{
...
void UiRuntimeNode::update_mapping_status(
...
void UiRuntimeNode::update_location_event(const savo_msgs::msg::LocationEvent & message)
{
  mapping_seen_ = true;
  last_mapping_update_ = std::chrono::steady_clock::now();
...
void UiRuntimeNode::update_live_state(const std::string & channel, const std::string & text)
{
  ...
  if (channel == "navigation") {
    ...
    if (
      !voice_has_priority &&
      active_screen_ != UiScreen::SafetyStop)
    {
      if (
        navigation_phase_ ==
        NavigationPhase::Idle)
      {
        ...
        transition_to(UiScreen::Navigation);
      }
    }
...
void UiRuntimeNode::update_power_source(
...
void UiRuntimeNode::update_power_status(const std::string & text)
{
...
bool UiRuntimeNode::power_source_is_stale(
...
void UiRuntimeNode::configure_runtime()
{
  ...
  loop_timer_ = create_wall_timer(
    period_ns,
    std::bind(&UiRuntimeNode::loop_callback, this));
}

void UiRuntimeNode::update_runtime(const double dt_seconds)
{
  ...
  if (
    active_screen_ == UiScreen::Navigation &&
    voice_phase_ == VoicePhase::Idle &&
    (
      navigation_phase_ == NavigationPhase::Arrived ||
      navigation_phase_ == NavigationPhase::Error
    ) &&
    navigation_phase_elapsed_seconds_ >= 4.0)
  {
    transition_to(UiScreen::Home);
  }
...
void UiRuntimeNode::render_current_screen()
{
...
void UiRuntimeNode::render_intro()
{
...
void UiRuntimeNode::render_intro_overlay(const float progress)
{
  const float phase =
...
void UiRuntimeNode::render_home()
{
  canvas_.clear(ColorRgb{2U, 8U, 22U});
...
void UiRuntimeNode::render_home_glow()
{
...
void UiRuntimeNode::render_home_dashboard()
{
  render_home_top_bar();
...
void UiRuntimeNode::render_home_top_bar()
{
...
void UiRuntimeNode::render_home_left_menu()
{
  const int x = 14;
  const int y = 70;
  const int w = 74;
...
void UiRuntimeNode::render_home_status_panel()
{
  const int x = 604;
  const int y = 104;
  const int w = 176;
...
void UiRuntimeNode::render_page_shell(
...
void UiRuntimeNode::render_voice_page(
  const VoicePhase phase)
{
  ...
  v2::VoicePhase v2_phase{v2::VoicePhase::Idle};

  switch (phase) {
    case VoicePhase::Idle:
      v2_phase = v2::VoicePhase::Idle;
      break;

    case VoicePhase::Listening:
      v2_phase = v2::VoicePhase::Listening;
      break;

    case VoicePhase::Thinking:
      v2_phase = v2::VoicePhase::Thinking;
      break;

    case VoicePhase::Speaking:
      v2_phase = v2::VoicePhase::Speaking;
      break;

    case VoicePhase::Error:
      v2_phase = v2::VoicePhase::Error;
      break;
  }

  v2::VoiceState state;
  state.phase = v2_phase;

  state.input_level = std::clamp(
    voice_ui_.input_level_percent / 100.0,
    0.0,
    1.0);
  ...
  const double animation_dt =
    1.0 / std::max(1.0, config_.loop_hz);

  v2_voice_animation_.update(
    animation_dt,
    v2_phase,
    state.input_level);

  if (!v2_voice_renderer_) {
    throw std::runtime_error(
      "V2 Voice renderer is not initialized");
  }

  v2_voice_renderer_->render(
    canvas_,
    state,
    v2_voice_animation_.frame());
}
...
void UiRuntimeNode::render_navigation_page(
  const NavigationPhase phase)
{
  ...
  v2::NavigationPhase v2_phase{
    v2::NavigationPhase::Idle};

  switch (phase) {
    case NavigationPhase::Idle:
      v2_phase = v2::NavigationPhase::Idle;
      break;

    case NavigationPhase::Preparing:
      v2_phase = v2::NavigationPhase::Preparing;
      break;

    case NavigationPhase::Navigating:
      v2_phase = v2::NavigationPhase::Navigating;
      break;

    case NavigationPhase::Paused:
      v2_phase = v2::NavigationPhase::Paused;
      break;

    case NavigationPhase::Arrived:
      v2_phase = v2::NavigationPhase::Arrived;
      break;

    case NavigationPhase::Error:
      v2_phase = v2::NavigationPhase::Error;
      break;
  }

  v2::NavigationState state;

  state.phase = v2_phase;
  state.goal_id = navigation_destination_;
  state.status = navigation_ui_.message;
  state.result = navigation_ui_.error_message;

  if (!v2_navigation_renderer_) {
    throw std::runtime_error(
      "V2 Navigation renderer is not initialized");
  }
...
void UiRuntimeNode::render_status_page()  // NOLINT(readability/fn_size)
{
...
void UiRuntimeNode::seed_status_preview_data()
{
...
void UiRuntimeNode::render_power_page()  // NOLINT(readability/fn_size)
{
...
void UiRuntimeNode::seed_power_preview_data()
{
...
void UiRuntimeNode::maybe_export_power_live_preview()
{
  if (
...
void UiRuntimeNode::render_placeholder_screen(
  const std::string & title,
...
void UiRuntimeNode::present_if_enabled()
{
  if (!config_.enable_framebuffer) {
    return;
  }
...
void UiRuntimeNode::request_status_view(
...
void UiRuntimeNode::queue_status_view_after_tts(
  const StatusView requested_view)
{
...
void UiRuntimeNode::request_screen(
...
void UiRuntimeNode::queue_screen_after_tts(
  const UiScreen requested_screen)
{
...
void UiRuntimeNode::handle_tts_finished()
{
  voice_phase_ = VoicePhase::Idle;
...
void UiRuntimeNode::transition_to(const UiScreen next_screen)
{
...
void UiRuntimeNode::loop_callback()
{
  const auto now = std::chrono::steady_clock::now();
...
int UiRuntimeNode::clamp_int(const int value, const int min_value, const int max_value)
{
  return std::clamp(value, min_value, max_value);
}

std::string UiRuntimeNode::bool_text(const bool value)
{
