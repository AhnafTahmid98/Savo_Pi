...
class CoreUpsNode final : public rclcpp::Node
{
public:
  CoreUpsNode()
  : rclcpp::Node(constants::kCoreUpsNodeName)
  {
    load_params();
...
private:
  void load_params()
  {
    ...
    params_.source = BatterySource::CORE_UPS;
  }
  ...
  {
    if (!ensure_driver()) {
      return make_error_reading(BatterySource::CORE_UPS, last_error_);
    }
  ...
  {
    const auto reading = read_core_ups();
