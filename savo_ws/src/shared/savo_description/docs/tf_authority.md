...
The RealSense driver must use `publish_tf: false`; its fixed model frames are owned by `robot_state_publisher`. `savo_head` must not publish `base_link -> pantilt_mount_link`.
